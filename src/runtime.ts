...
const nilAttributeStore = new WeakMap<object, Map<string, ([string, string][] | undefined)[]>>();
...
const transferLexicals = (walked: unknown, parsed: unknown): void => {
  ...
  const nilRecord = nilAttributeStore.get(walked);
  if (nilRecord !== undefined) {
    nilAttributeStore.delete(walked);
    nilAttributeStore.set(parsed, nilRecord);
  }
...
const readOccurrence = (
  ...
  nilAttributes?: [string, string][] | undefined;
} => {
...
// Generated files are only supported with the runtime version that generated
// them. Warn (once per process) when a root schema's version stamp predates a
// breaking change: a different major, or any difference while both are 0.x.
let generatorMismatchWarned = false;
const warnOnGeneratorMismatch = (meta: XmlMeta): void => {
  if (generatorMismatchWarned || meta.generatedBy === undefined) {
    return;
  }
  const generated = /^xsd-to-zod@(\d+\.\d+\.\d+(?:-.+)?)$/.exec(meta.generatedBy)?.[1];
  if (generated === undefined || generated === PACKAGE_VERSION) {
    return;
  }
  const generatedMajor = generated.split(".")[0];
  const runtimeMajor = PACKAGE_VERSION.split(".")[0];
  if (generatedMajor === runtimeMajor && runtimeMajor !== "0") {
    return;
  }
  generatorMismatchWarned = true;
  console.warn(
    `xsd-to-zod: this schema was generated by xsd-to-zod@${generated} but the runtime is xsd-to-zod@${PACKAGE_VERSION}. ` +
      "Generated files are only supported with the version that generated them — regenerate to upgrade.",
  );
};
...
const writeObjectFields = (
  ...
  const emitElementOccurrence = (
    ...
    if (item === null) {
      ...
      const nilAttrs = (nilAttributeStore.get(obj)?.get(key)?.[i] ?? []).map(
        ([clark, lexical]) =>
          `${elementName(clark, ctx.prefixMap, ctx.qnameNs)}="${escapeXmlAttrChars(escapeXml(lexical))}"`,
      );
    ...
    // on re-parse.
    const simpleXsiType =
      fieldMeta.anySimpleType === true ? openXsiTypeStore.get(obj)?.get(key)?.[i] : undefined;
    const typeAttr =
    ...
    for (const [key, fieldMeta] of Object.entries(fields)) {
      ...
      occurrences.forEach((occ, i) => {
        ...
        out.push({
          ...
          nilAttributes: nilRecord?.get(key)?.[i],
        });
      });
    }
  }
  for (const [key, entryValue] of Object.entries(obj)) {
    ...
    occurrences.forEach((occ, i) => {
      out.push({
        ...
        qname: key as QName,
        path: [...node.path, key, ...(isArray ? [i] : [])],
        xsiType: openXsiTypeStore.get(obj)?.get(key)?.[i],
      });
    });
  }
  return out;
...
const identityAttributeNode = (node: IdentityNode, qname: QName): IdentityNode | undefined => {
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    ...
    return found === undefined
      ? undefined
      : { value: found[1], schema: undefined, qname, path: node.path };
  }
  const obj = value as Record<string, unknown>;
  const fields = node.schema === undefined ? undefined : findFieldsMeta(node.schema);
  if (fields !== undefined) {
