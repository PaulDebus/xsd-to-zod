...
const KNOWN_ANOMALIES = new Map<string, string>([
  ['suntest/identitytestsuitetest001/test.2.n', 'identity constraints unsupported — both tiers accept'],
  ['SimpleType/ste110/ste110.i', 'circular union self-reference (codegen: undefined union member)'],
  ['Element/elemZ016/elemZ016.i', 'both tiers accept (needs triage — likely XSD 1.0/1.1 expectation difference)'],
...
const libxmlValidates = async (xml: string, xsdFiles: string[]): Promise<boolean> => {
  ...
  for (const { file } of matching.length > 0 ? matching : withNs) {
    try {
      if ((await validateXml(xml, readXmlFile(file), { url: file })).valid) return true;
    } catch {
...
const record = (anchor: string, outcome: Outcome, key: string): void => {
  ...
  if (outcome === 'lenient') lenientCases.push(key);
};
    ...
    JSON.stringify(
      {
        ...
        bySpecSection: Object.fromEntries([...stats.entries()].sort(([a], [b]) => a.localeCompare(b))),
...
describe('W3C negative tests (invalid instances)', () => {
  ...
  const cases = discoverInvalidCases(TEST_SETS.map(ref => typeof ref === 'string' ? path.join(W3C_DIR, ref) : { ...ref, file: path.join(W3C_DIR, ref.file) }));
  ...
  for (const c of cases) {
    ...
    const body = async () => {
      ...
      for (const anchor of anchors) record(anchor, outcome, key);
